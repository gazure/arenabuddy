...
fn DraftRow(draft: Draft) -> Element {
    rsx! {
        Link {
            ...
            td { class: "py-3 px-4 border-b border-gray-200 dark:border-gray-700", "{draft.format()}" }
            td { class: "py-3 px-4 border-b border-gray-200 dark:border-gray-700",
                span { class: if draft.status() == "DraftStatus_Complete" { "text-amber-600 dark:text-amber-400" } else { "text-yellow-600 dark:text-yellow-400" },
                    if draft.status() == "DraftStatus_Complete" { "COMPLETE" } else { "IN PROGRESS" }
                }
            }
            td { class: "py-3 px-4 border-b border-gray-200 dark:border-gray-700 text-gray-500",
    ...
    rsx! {
        div { class: "container mx-auto px-4 py-8 max-w-5xl",
            div { class: "flex justify-between items-center mb-6",
                h1 { class: "text-2xl font-bold text-gray-900 dark:text-gray-100", "Draft History" }
                button {
            ...
            div { class: "bg-white dark:bg-gray-800 rounded-lg border border-gray-200 dark:border-gray-700 overflow-hidden",
                match data.as_ref() {
                    ...
                    Some(Err(err)) => rsx! {
                        div { class: "bg-red-100 dark:bg-red-900/30 border border-red-300 dark:border-red-700 text-red-700 dark:text-red-300 px-4 py-3 rounded m-4",
                                    ...
                                    table { class: "min-w-full table-auto",
                                        thead {
                                            tr { class: "bg-gray-50 dark:bg-gray-900 text-left",
                                                ...
                                                th { class: "py-3 px-4 font-semibold text-gray-600 dark:text-gray-400", "Format" }
                                                th { class: "py-3 px-4 font-semibold text-gray-600 dark:text-gray-400", "Status" }
                                                th { class: "py-3 px-4 font-semibold text-gray-600 dark:text-gray-400", "Date" }
                                            }
                                        }
                                        tbody {
