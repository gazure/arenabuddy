...
pub fn Pagination(current_page: Signal<usize>, total_pages: usize, total_items: usize, page_size: usize) -> Element {
    ...
    rsx! {
        div { class: "flex justify-between items-center py-3 px-4 border-b border-gray-200 dark:border-gray-700 bg-gray-50 dark:bg-gray-900",
            p { class: "text-sm text-gray-600 dark:text-gray-400",
                "Showing {start}–{end} of {total_items}"
            }
            div { class: "flex items-center space-x-2",
                button {
                    class: "px-3 py-1 rounded text-sm bg-gray-200 dark:bg-gray-700 hover:bg-gray-300 dark:hover:bg-gray-600 disabled:opacity-50 disabled:cursor-not-allowed transition-colors duration-150",
                    disabled: page == 0,
